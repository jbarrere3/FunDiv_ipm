...
#' Plot traits PCA with species not included in simulations
#' @param traits dataframe containing trait value per species
#' @param traits_rec recruitment traits per species
#' @param species_list dataset generated by get_species_list() function
#' @param file.in name including path of the file to save
plot_traits_pca12_all <- function(traits, traits_rec, species_list, file.in){
  ...
  data.ind = data.frame(species = data_traits$species, 
                        pca1 = get_pca_ind(pca)[[1]][, 1], 
                        pca2 = get_pca_ind(pca)[[1]][, 2])  %>%
    mutate(insim = ifelse(species %in% unique(species_list$species), "yes", "no")) %>%
    mutate(species = gsub("\\_", "\\ ", species))
  ...
  plot.out = data.ind %>%
    ...
    geom_segment(data = data.var, aes(x = 0, xend = pca1, y = 0, yend = pca2), 
                 arrow = arrow(length = unit(0.3, "cm"))) + 
    geom_point(size = 2, shape = 21, aes(fill = insim), color = "black", alpha = 0.5) +
    geom_text(data = data.var, aes(label = var, x = pca1.txt, y = pca2.txt)) +
    # Frame of the graph
    geom_rect(aes(xmin = range.x[1], xmax = range.x[2], ymin = range.y[1], ymax = range.y[2]), 
              color = "black", fill = NA) +
    ...
    geom_segment(x = 0, xend = 0, y = range.y[1], yend = range.y[2], linetype = "dashed") +
    # Segment to connect points to species name
    geom_segment(aes(xend = pca1_x, color = insim), yend=range.y[2],  
                 linetype = "dotted", inherit.aes = TRUE, size = 0.3) +
    geom_segment(aes(yend = pca2_y, color = insim), xend=range.x[2], 
                 linetype = "dotted", inherit.aes = TRUE, size = 0.3) +
    # Axis label
    xlab(paste0("PCA1 (", round(summary(pca)$importance[2, 1]*100, digits = 2), "%)",
                "\nHigh survival <--> High growth")) +
    ...
    # text of the x top axis
    geom_text(angle = 90, size = 2, 
              aes(x = pca1_x, y = pca1_y, label = species, color = insim), 
              inherit.aes = TRUE, vjust = 0, hjust = 0, fontface = "italic") + 
    # text of the y top axis
    geom_text(size = 2, 
              aes(x = pca2_x, y = pca2_y, label = species, color = insim), 
              inherit.aes = TRUE, vjust = 0, hjust = 0, fontface = "italic") + 
    # Theme
    theme(panel.background = element_blank(), 
          panel.grid = element_blank(), 
    ...
    geom_text(y = mean(range.y), x = range.x[1] - 3*space.frac*diff(range.x), angle = 90,
              label = "High recruitment <--> Low recruitment", fontface = "bold", size = 3) + 
    scale_fill_manual(values = c('yes' = "#219EBC", 'no' = "#CCC5B9")) + 
    scale_color_manual(values = c('yes' = "#219EBC", 'no' = "#CCC5B9"))
...
#' Function to remove all simulations with Salix caprea from the dataset
#' @param data_model dataframe used or all analysis
remove_salix = function(data_model){
  
  # Initialize the vector that will contain ID of simulations with salix
  vec.ID.forest = c()
  
  # Loop on all forest compositions
  for(i in 1:dim(data_model)[1]){
    # List all species in the forest in a vector
    vec.sp.i = unlist(strsplit(data_model$forest.composition[i], "\\."))
    # If salix is in the vector, get the ID of the forest
    if("Salix_caprea" %in% vec.sp.i){
      vec.ID.forest = c(vec.ID.forest, data_model$ID.forest[i]) 
    } 
  }
  # Remove forests with salix
  out = subset(data_model, !(ID.forest %in% vec.ID.forest))
  # Other option : remove only monospecific stands of salix
  out2 = subset(data_model, forest.composition != "Salix_caprea")
  # Return output
  return(out2)
}
